    ...
    try {
        ...
        if ($usu === false) {
            ...
            try {
                ...
                if ($idCarro == FALSE) {
                    // Si no hay carrito pendiente creamos uno nuevo en BD
                    $sqlInsert = "INSERT INTO carro (Usuario, Fecha, Enviado, Total) VALUES (?, ?, 0, 0.00)";
                    $stmtInsert = $bd->prepare($sqlInsert);
                    $stmtInsert->execute([$usuario_email, date('Y-m-d H:i:s')]);
                    $newCodCarro = $bd->lastInsertId();
                    
                    $_SESSION['CodCarro'] = $newCodCarro; // Guardar el ID del carrito
                    $_SESSION['carrito'] = [];
                } else {
